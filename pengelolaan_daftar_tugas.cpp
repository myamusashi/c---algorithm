...
void CompletedTask() {
/* 
Cara kode ini bekerja, data array yang dipilih untuk selesai akan di hapus di function mainData 
dan akan dipindahkan ke tugasSelesai 
*/
std::string completed_task;
    ...
    if (!(std::cin >> nomorTugasSelesai)) {
        ...
        // Membersihkan buffer input untuk tidak menciptakan unexpected error ketika user salah memasukkan input
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    } else {
        try {
